...
### 6. 🚀 Ni-Oh Hub Mode — Trợ Lý Realtime Độc Lập (`src/hub/hub_app.py`)
Ni-Oh Hub Mode là **phiên bản realtime overlay** chạy độc lập, **không cần Discord** — Ni-Oh sống ngay trên desktop của Sếp:

* 🟣 **Icon nổi 70×70px** luôn hiển thị trên cùng (Always-on-Top), có hiệu ứng nhấp nháy khi đang nói.
* 💬 **Khung chat popup** 380×520px bật ra khi click icon hoặc nhấn `Ctrl+Shift+N`.
* 🎙️ **Giọng Việt chuẩn streaming** với [VieNeu-TTS](https://github.com/pnnbao97/VieNeu-TTS) (23 giọng preset bản địa, không phải translate từ Anh ngữ). Chunk đầu tiên ra sau chỉ ~0.4s.
* ⚡ **Phím tắt global**: `Ctrl+Shift+N` hoặc `Ctrl+Shift+C` để toggle khung chat, `Esc` để thu nhỏ về icon.
* 🔌 **Mặc định dùng model đang chat với Sếp Neito** (`minimax/minimax-m3` qua OpenRouter) — kế thừa "bộ não" của Ni-Oh cho realtime. Người dùng khác có thể override trong `.env`:
  ```ini
  # .env — chọn model khác cho Hub
  NIOH_MODEL=openai/gpt-4o-mini
  NIOH_PROVIDER=openrouter
  ```
* 🚀 **Khởi động 1-click**: Chạy `run.bat` ở thư mục gốc → icon hiện ra ngay, không cần Discord, không cần web UI, không cần terminal.
* 🔇 **Không cần console**: dùng `pythonw.exe` trong venv Hermes, không hiện cửa sổ đen.
