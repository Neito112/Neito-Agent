...
const workStudyProtocol = require('./protocols/work_study_protocol.js');
const liveCompanionProtocol = require('./protocols/live_companion_protocol.js');
...
const PROTOCOLS = {
  ...
  work_study: {
    ...
    description: 'Hỗ trợ tính toán học thuật, kỹ thuật, tự động hóa công thức Google Sheets/Excel.',
    module: workStudyProtocol
  },
  live_companion: {
    id: 'live_companion',
    ...
    description: 'Trợ lý live stream: 1-2 câu ngắn gọn, không chen ngang combat, đồng bộ HUD Overlay.',
    module: liveCompanionProtocol
  },
  general: {
    id: 'general',
...
async function setProtocol(protocolQuery) {
  ...
  if (target) {
    ...
    console.log(`[ProtocolManager] Kích hoạt giao thức: ${target.name}. Đang kích hoạt tiến trình cập nhật tri thức mới nhất...`);

    // 1. Trigger protocol module auto-update
    if (target.module && typeof target.module.fetchLatestUpdates === 'function') {
      target.module.fetchLatestUpdates().catch(err => console.warn(`[ProtocolManager] Module update notice:`, err.message));
    }

    // 2. Trigger 24/7 Local Knowledge Daemon sweep for this topic
    try {
      const kd = require('./knowledge_daemon.js');
      const searchQuery = target.searchQuery || `${target.name} latest update patch notes meta`;
      kd.updateTopicKnowledge(target.id, target.name, searchQuery).catch(() => {});
    } catch (_) {}
...
module.exports = {
  ...
  work_study: workStudyProtocol,
  live_companion: liveCompanionProtocol
};